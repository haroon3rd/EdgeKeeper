...
public class GNSCommand extends CommandPacket {
   ...
   * @param accountAlias for GUID
   * @param GUID account password
   *
   * @return CommandPacket
 ...
 * @throws KeyStoreException 
  ...
  public static final CommandPacket createAccountWithP12Certificate(
		  KeyStore keyStore, String keyStorePassword, String accountAlias, String accountPassword) throws CertificateEncodingException, ClientException, NoSuchAlgorithmException, UnrecoverableKeyException, KeyStoreException  {
	
	  // get certificate from keystore
    X509Certificate cert = (X509Certificate) keyStore.getCertificate(accountAlias);
    ...
    // load private key from file
    PrivateKey privatekey = (PrivateKey) keyStore.getKey( accountAlias, keyStorePassword.toCharArray());
    ...
    @SuppressWarnings("deprecation") // FIXME
            ...
            // so overwrite the local hashmap with new values
            GuidEntry guidEntry = createGuidEntryWithCertificate(GNSClient.getGNSProvider(),
            accountAlias, publickey, privatekey);
    return accountGuidCreateInternalWithCertificate(accountAlias, accountPassword, CommandType.RegisterAccountWithCertificate, guidEntry, cert);
  }
   ...
   *
   * @param certificateFileName
   * @param Certificate password
   * @param GUID account alias
   * @param GUID account password
   *
   * @return CommandPacket
 * @throws KeyStoreException 
 ...
 * @throws CertificateException 
  ...
  public static final CommandPacket createAccountWithP12Certificate(
          String p12FileName, String keyStorePassword, String accountAlias, String accountPassword) throws UnrecoverableKeyException, ClientException, NoSuchAlgorithmException, KeyStoreException, IOException, CertificateException   {
	  
	// Load the Keystore  
	  KeyStore keyStore = KeyStore.getInstance("PKCS12");
	  FileInputStream fin = new FileInputStream( p12FileName);
	  keyStore.load(fin, keyStorePassword.toCharArray());
	  fin.close();
	  
	  return createAccountWithP12Certificate(keyStore, keyStorePassword, accountAlias, accountPassword);
