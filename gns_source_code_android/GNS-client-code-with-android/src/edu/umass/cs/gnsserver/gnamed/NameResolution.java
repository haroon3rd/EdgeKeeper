...
public class NameResolution {
  ...
  public static Message lookupGnsServer(InetAddress addr, Message query, ClientRequestHandlerInterface handler) {
    ...
    final String domainName = querytoStringForGNS(rawName);
	  ...
	  System.out.println("\n\n[LENSS] looking for domain: "+domainName);
     ...
     */
	  ...
	  //System.out.println("\n[LENSS]: " +addr.getHostAddress().toString());
	  if (addr==null) 
		  fieldResponseJson = lookupGuidField("127.0.0.1", query.getHeader().getID(), domainName, null, fields, handler);
	  else
		  fieldResponseJson = lookupGuidField(addr.getHostAddress().toString(), query.getHeader().getID(), domainName, null, fields, handler);
	  	  
	  if (fieldResponseJson == null) {
	    NameResolution.getLogger().log(Level.FINE, "GNS lookup for domain {0} failed.", domainName);
	    System.out.println("GNS lookup for domain {0} failed."+ domainName);
	    return errorMessage(query, Rcode.NXDOMAIN);
	  }
	  
	  NameResolution.getLogger().log(Level.FINE, "fieldResponse all fields (NS, MX, CNAME, A): {0}", fieldResponseJson.toString());
	  System.out.println( "fieldResponse all fields (NS, MX, CNAME, A): "+ fieldResponseJson.toString());
