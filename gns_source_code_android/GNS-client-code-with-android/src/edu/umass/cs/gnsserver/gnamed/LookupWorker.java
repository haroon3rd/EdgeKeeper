...
public class LookupWorker implements Runnable {
  ...
  private Message generateReply(Message query) {
	  System.out.println("\n\n[LENSS] DNS quesry received: "+query.toString());
    long startTime = System.currentTimeMillis();
    ...
    if ((errorMessage = NameResolution.checkForErroneousQueries(query)) != null) {
    	System.out.println("\n\n[LENSS] Weired message. Error: "+errorMessage.toString());
      return errorMessage;
    }
    ...
    System.out.println("\n\n[LENSS] DNS query still needs procccessing");
    ...
    for (int i=0; i<tasks.size(); i++) { // this is just doing things twice btw
      try {
        ...
        System.out.println("Problem handling lookup task: {0}"+e.toString());
