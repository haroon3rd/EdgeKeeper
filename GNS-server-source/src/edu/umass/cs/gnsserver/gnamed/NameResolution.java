...
public class NameResolution {
	...
	public static Message forwardToDnsServer(SimpleResolver dnsServer, Message query) {
		try {
			// Suman's edit
			dnsServer.setTimeout(3);

			/*
			 * //Suman is trying to bug the internet disconnection problem.
			 * NameResolution.getLogger().log(Level.FINE,"LENSS. Delaying the DNS"); try {
			 * Thread.sleep(3000); } catch (InterruptedException e) {
			 * NameResolution.getLogger().log(Level.FINE,"LENSS. Problem in sleep"); }
			 * finally { NameResolution.getLogger().log(Level.FINE,"LENSS. Delaying ends");
			 * }
			 */

			NameResolution.getLogger().log(Level.FINE, "LENSS. sending query to DNS.  " + query.toString());
			Message dnsResponse = dnsServer.send(query);
			NameResolution.getLogger().log(Level.FINE,
					"LENSS. DNS response {0} with {1} answer, {2} authoritative and {3} additional records",
			...
			NameResolution.getLogger().log(Level.FINE, "LENSS. got reply from DNS  ");

			if (isReasonableResponse(dnsResponse)) {
				NameResolution.getLogger().log(Level.FINE, "LENSS. Outgoing response from DNS: {0}",
						dnsResponse.toString());
				return dnsResponse;
			}
		} catch (IOException e) {
			NameResolution.getLogger().log(Level.WARNING, "LENSS. DNS resolution failed for {0}: {1}",
					new Object[] { query, e });
		}
	...
	public static Message forwardToGnsServer(SimpleResolver gnsServer, Message query) {
		try {
			NameResolution.getLogger().log(Level.FINE, "LENSS. sending query to GNS.  " + query.toString());
			Message dnsResponse = gnsServer.send(query);
			NameResolution.getLogger().log(Level.FINE,
					"[LENSS] GNS-DNS response {0} with {1} answer, {2} authoritative and {3} additional records",
					new Object[] { Rcode.string(dnsResponse.getHeader().getRcode()),
							dnsResponse.getSectionArray(Section.ANSWER).length,
							dnsResponse.getSectionArray(Section.AUTHORITY).length,
							dnsResponse.getSectionArray(Section.ADDITIONAL).length });
			if (isReasonableResponse(dnsResponse)) {
				NameResolution.getLogger().log(Level.FINE, "Outgoing response from GNS-DNS: {0}",
						dnsResponse.toString());
				return dnsResponse;
			}
		} catch (IOException e) {
			NameResolution.getLogger().log(Level.WARNING, "GNS-DNS resolution failed for {0}: {1}",
					new Object[] { query, e });
		}
		return errorMessage(query, Rcode.NXDOMAIN);
	}
	...
	public static Message lookupGnsServer(InetAddress addr, Message query, ClientRequestHandlerInterface handler) {
		NameResolution.getLogger().log(Level.FINE, "LENSS. Looking own GNS records");
		...
		NameResolution.getLogger().log(Level.FINE, "LENSS looking for domain: " + domainName);
		...
		/**
		 * The query type or domain name can't be null, otherwise return an error
		 * message
		 */
		if (Type.string(fieldName) == null || domainName == null) {
			NameResolution.getLogger().log(Level.WARNING, "LENSS Domain name null");
			return errorMessage(query, Rcode.NXDOMAIN);
		}
		NameResolution.getLogger().log(Level.FINE, "LENSS. Trying GNS lookup for domain {0}, type {1}",
				new Object[] { domainName, Type.string(fieldName) });
		...
		// Suman's edit. QR field should be reseted for respose
		 ...
		 */
		ArrayList<String> fields = new ArrayList<>(Arrays.asList("A", "AAAA", "NS", "CNAME", "SOA", "PTR", "MX"));

		/**
		 * <p>
		 * RFC 1034: the additional section "carries RRs(Resource Records) which may be
		 * helpful in using the RRs in the other section" RFC 2181: data you put in the
		 * additional section can never be promoted into real answers.
		 * 
		 * <p>
		 * When a DNS client needs to look up a name used in a program, it queries DNS
		 * servers to resolve the name. Each query message the client sends contains
		 * three pieces of information, specifying a question for the server to answer:
		 * 1. A specified DNS domain name, stated as a fully qualified domain name
		 * (FQDN). 2. A specified query type, which can either specify a resource record
		 * (RR) by type or a specialized type of query operation. 3. A specified class
		 * for the DNS domain name. For DNS servers running the Windows operating
		 * system, this should always be specified as the Internet (IN) class.
		 * 
		 * <p>
		 * The information is retrieved from GNS based on the queried domain.
		 * <p>
		 * The response is constructed based on the query type, 1. A: return A records
		 * in ANSWER section, NS records in AUTHORITY section, A records of name servers
		 * in ADDITIONAL section 2. NS: return NS records in ANSWER section, A records
		 * of name servers in ADDITIONAL section 3. MX: return MX records in ANSWER
		 * section, NS records in AUTHORITY section, A record of name servers in
		 * ADDITIONAL section 4. CNAME: return CNAME records in in ANSWER section, NS
		 * records in AUTHORITY section, A record of name servers in ADDITIONAL section
		 * 
		 * Records in ADDITIONAL section is not required, we do a best-effort resolution
		 * for the names in ADDITIONAL section.
		 */
		long resolveStart = System.currentTimeMillis();

		JSONObject fieldResponseJson = null;

		if (addr == null)
			fieldResponseJson = lookupGuidField("127.0.0.1", query.getHeader().getID(), domainName, null, fields,
					handler);
		else
			fieldResponseJson = lookupGuidField(addr.getHostAddress().toString(), query.getHeader().getID(), domainName,
					null, fields, handler);

		if (fieldResponseJson == null) {
			NameResolution.getLogger().log(Level.FINE, "LENSS. GNS lookup for domain {0} failed.", domainName);
			return errorMessage(query, Rcode.NXDOMAIN);
		}

		NameResolution.getLogger().log(Level.FINE, "LENSS. fieldResponse all fields (NS, MX, CNAME, A): {0}",
				fieldResponseJson.toString());
		...
		case Type.NS: {
			...
			break;
		// Suman's edit. AAAA record should be treated separately,but it would require
		// significantchanges. That's why, I am doing this:

		case Type.AAAA:
			// Get A records from retrieved GNS record
			JSONArray aaaaList = getAAAARecordsFromAAAAField(fieldResponseJson, domainName);
			if (aaaaList != null) {
				for (int i = 0; i < aaaaList.length(); i++) {
					try {
						response.addRecord((Record) aaaaList.get(i), Section.ANSWER);
					} catch (JSONException e) {
						// trash the record
					}
				}
		...
		case Type.MX: {
			...
			NameResolution.getLogger().log(Level.FINE, "MX record for domain {0} is {1}",
					new Object[] { domainName, obj });
			if (obj != null) {
				try {
		...
		default:
			...
			NameResolution.getLogger().log(Level.FINER, "Suman Invalid type found {0}", response.toString());
			return errorMessage(query, Rcode.NOTIMPL);
		}

		DelayProfiler.updateDelay("ResolveName", resolveStart);
		// response.getHeader().setFlag(Flags.QR);
		NameResolution.getLogger().log(Level.FINER, "LENSS. Outgoing response from GNS: {0}", response.toString());
		return response;
	}
	...
	private static JSONArray getARecordsFromAField(JSONObject fieldResponseJson, String nameToResolve) {
		...
		/**
		 * Format of A record in GNS: { "A": { "record": String[ip1, ip2, ...], "ttl":
		 * int } }
		 */
		if (fieldResponseJson.has("A")) {
			...
			for (int i = 0; i < records.length(); i++) {
				try {
					...
					ARecord gnsARecord = new ARecord(new Name(nameToResolve), DClass.IN, ttl,
	...
	/**
	 * Suman's edit. new code retrieve all AAAA records from A field of a JSON
	 * object
	 * 
	 * @return
	 */
	private static JSONArray getAAAARecordsFromAAAAField(JSONObject fieldResponseJson, String nameToResolve) {
		NameResolution.getLogger().fine("Inside AAA solver: " + fieldResponseJson.toString());

		JSONArray aList = new JSONArray();
		/**
		 * Format of A record in GNS: { "AAAA": { "record": String[ip1, ip2, ...],
		 * "ttl": int } }
		 */
		if (fieldResponseJson.has("AAAA")) {
			JSONArray records = null;
			int ttl = 60;
			try {
				JSONObject recordObj = fieldResponseJson.getJSONObject("AAAA");
				records = recordObj.getJSONArray(ManagedDNSServiceProxy.RECORD_FIELD);
				ttl = recordObj.getInt(ManagedDNSServiceProxy.TTL_FIELD);
			} catch (JSONException e) {
				// something is wrong with the JSON object, return null
			...
			for (int i = 0; i < records.length(); i++) {
				try {
					String ip = records.getString(i);
					NameResolution.getLogger().warning("Parsing this IP: " + ip);
					NameResolution.getLogger().warning("Parsing this IP: " + Inet6Address.getByName(ip));

					AAAARecord gnsARecord = new AAAARecord(new Name(nameToResolve), DClass.IN, ttl,
	...
	/**
	 * retrieve all NS records and the corresponding A records from NS field of a
	 * JSON object. the key "NS" contains a list of all NS records the key "A"
	 * contains a list of all A records, which must be put into ADDITIONAL section
	 * 
	 */
	private static JSONObject getNSRecordsFromNSField(JSONObject fieldResponseJson, String nameToResolve) {
		...
		/**
		 * Format of NS record in GNS: { "NS": { "record":[(ns1, addr1), (ns2, addr2),
		 * ...], "ttl":int } }
		 * 
		 */
		if (fieldResponseJson.has("NS")) {
			...
			for (int i = 0; i < records.length(); i++) {
				try {
					...
					// It must be an absolute name, i.e., the string must be ended with a dot, e.g.,
					// example.com.
					if (!ns.endsWith(".")) {
						ns = ns + ".";
					}
					...
					nsList.put(nsRecord);
					// address can be null as the domain name might use other service as its name
					// server
					if (record.length() == 2) {
	...
	/**
	 * retrieve MX record from MX field of a JSON object the key "MX" contains a
	 * list of all MX records the key "A" contains a list of all A records, which
	 * must be put into ADDITIONAL section
	 * 
	 * @return
	 */
	...
	/**
	 * Lookup the field or fields in the guid. Returns a JSONObject containing the
	 * fields and values or null if the domainName doesn't exist.
	 * 
	 * @param addr
	 * @param id
	 ...
	 */
	public static JSONObject lookupGuidField(String addr, int id, String domain, String field, ArrayList<String> fields,
			ClientRequestHandlerInterface handler) {
		/**
		 * Querying multiple types together is allowed in DNS protocol, but practically
		 * not supported. Therefore, no need for us to implement support for multi-type
		 * query.
		 */
		...
		if (guid != null) {
			...
			try {
				...
				NameResolution.getLogger().log(Level.FINE, "Fetching record failed for {0}: {1}",
						new Object[] { domain, e });
			}
		} else {
	...
	/**
	 * Look up the local dns server cache. Returns a {@link Message}.
	 *
	 * @param query
	 * @param dnsCache
	 * @return a Message
	...
	/**
	 * Returns true if the response looks ok. Checks for errors and also 0 length
	 * answers.
	 *
	 * @param dnsResponse
	...
	private static Message buildErrorMessage(Header header, int rcode, Record question) {
		...
		// Suman's edit. The response bit should be set This is very crucial.
		response.getHeader().setFlag(Flags.QR);
		for (int i = 0; i < 4; i++) {
	...
	/**
	 * Builds a pretty string showing the query and response. Used for debugging
	 * purposes.
	 *
	 * @param query
	 * @param response
	...
	/**
	 * Creates a string out of raw bytes[] from qname section of the DNS query
	 * message. This routine, when used, adds '.' at the end for using it for GNS
	 * lookups.
	 *
	 * @param qname
