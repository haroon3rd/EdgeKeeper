...
public class LookupWorker implements Runnable {
  ...
  public void run() {
    ...
    NameResolution.getLogger().log(Level.FINE, "[LENSS] Reply response message: {0}", response.toString());
  ...
  private Message generateReply(Message query) {
    ...
    if ((errorMessage = NameResolution.checkForErroneousQueries(query)) != null) {
    	NameResolution.getLogger().log(Level.WARNING, " Weired message. Error: "+errorMessage.toString());
      return errorMessage;
    }
	...
	//Suman is commenting this code	
    /*
		 *
		 * if (dnsCache != null) { Message tempQuery = (Message) query.clone(); Message
		 * result = NameResolution.lookupDnsCache(tempQuery, dnsCache); if
		 * (result.getHeader().getRcode() == Rcode.NOERROR) {
		 * NameResolution.getLogger().log(Level.FINE,
		 * "LENSS. Responding the request from cache {0}",
		 * NameResolution.queryAndResponseToString(query, result)); return result; } }
		 */
    ...
    NameResolution.getLogger().log(Level.FINE,"LENSS DNS query still needs procccessing");
 ...
 // A little bit of overkill for two tasks, but it's really not that much longer (if any) than
    ...
    for (int i=0; i<tasks.size(); i++) { // this is just doing things twice btw
      try {
        ...
        NameResolution.getLogger().log(Level.FINE, "LENSS. Obtained one result from lookupWorker "+result.toString());
    ...
    NameResolution.getLogger().log(Level.FINE, "LENSS. Finished executing the lookupworkers. ");
